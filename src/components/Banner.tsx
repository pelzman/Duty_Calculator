...
const Banner = ({
    exchangeRates,
    selectedRate, setSelectedRate, selectedCountryCode, setSelectedCountryCode }: { exchangeRates: RateData[], selectedRate: string, selectedCountryCode: string, setSelectedRate: (value: string) => void, setSelectedCountryCode: (value: string) => void }) => {
    ...
    return (
        <div className="md:flex md:justify-between md:items-center  text-black  px-6 md:px-12 py-2 space-y-4  md:space-y-0 rounded-b-lg">
            ...
            <div className=" space-y-3 md:w-[300px] md:flex md:gap-3 md:justify-between md:align-middle">

                <Select name="Countrycode" value={selectedCountryCode} onValueChange={(value) => setSelectedCountryCode(value)} required>
                    ...
                    <SelectContent className="  md-w-[200px] max-h-60 overflow-y-auto px-[20px] bg-white shadow-2xl " >
...

                        {
                            currencyCodes.map((code, index) => {
                                ...
                                return (
                                    <SelectItem key={index} value={code.toString()}>
                                        {code}
                                        {/* "NGN 1570" */}
                                    </SelectItem>
                                )
